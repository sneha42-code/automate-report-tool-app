...
const EnhancedReportTool = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      ...
      try {
        const data = await AttritionAnalysisService.getRecentReports();
  ...
  const handleFiles = async (files) => {
    ...
    try {
      ...
      const response = await AttritionAnalysisService.uploadFile(
        filesArray[0], 
  ...
  const handleGenerateClick = async () => {
    ...
    try {
      ...
      const response = await AttritionAnalysisService.generateReport(fileId);
      ...
      setGeneratedReport({
        ...
        url: AttritionAnalysisService.getDownloadUrl(fileId, response.report_file),
      });
    } catch (err) {
  ...
  const handleDownloadClick = async () => {
    ...
    try {
      if (generatedReport.url) {
        AttritionAnalysisService.downloadReport(generatedReport.id, generatedReport.name);
      }
    } catch (err) {
        ...
        <div className="platform-modules">
          <section className="data-module">
            <div className="module-header">
              <h3>Data Input  --- Output (file format  * excel) </h3>
...
export default EnhancedReportTool;